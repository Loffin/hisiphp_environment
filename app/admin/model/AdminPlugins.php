...
class AdminPlugins extends Model
{
    ...
    public function design($data = [])
    {
        ...
        if (in_array('controller', $dir_list) !== false) {
            $dir_list[] = 'view/controller/index';
        }
    ...
    public function mkExample($path = '', $data = [])
    {
        ...
        if (is_dir($path.'controller')) {
            $home = "<?php\nnamespace plugins\\".$data["name"]."\\controller;\nuse app\common\controller\Common;\ndefined('IN_SYSTEM') or die('Access Denied');\n\nclass Index extends Common\n{\n    public function index()\n    {\n        return ".'$this->fetch()'.";\n    }\n}";
            file_put_contents($path . 'controller'.DS.'Index.php', $home);
            file_put_contents($path.'view'.DS.'controller'.DS.'index'.DS.'index.php', '<?php defined("IN_SYSTEM") or die("Access Denied");/* 防止模板被盗 */?>');
        }
    }
